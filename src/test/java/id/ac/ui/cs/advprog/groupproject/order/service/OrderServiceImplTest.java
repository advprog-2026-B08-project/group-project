...
class OrderServiceImplTest {
    ...
    @BeforeEach
    void setUp() {
        orderService = new OrderServiceImpl(orderRepository, statusHistoryRepository, stockPort, paymentPort);
        buyerId = UUID.randomUUID();
        jastiperId = UUID.randomUUID();
        productId = UUID.randomUUID();
        productSnapshot = new ProductSnapshot(jastiperId, BigDecimal.valueOf(100000), "Produk Test");
    }
    ...
    void checkout_selfPurchase_releasesStockAndThrows() {
        ...
        ProductSnapshot selfSnapshot = new ProductSnapshot(sameId, BigDecimal.valueOf(50000), "Self Product");
    ...
    void findByJastiperId_returnsJastiperOrders() {
        when(orderRepository.findByJastiperId(jastiperId)).thenReturn(List.of(new Order(), new Order()));
    ...
    void updateStatus_validTransition_paidToPurchased() {
        ...
        Order result = orderService.updateStatus(orderId, OrderStatus.PURCHASED);
